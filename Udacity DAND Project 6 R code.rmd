...
output: 
  html_document:
    ...
    toc         : false              # get table of content
    toc_depth   : 3
    toc_float   : false
...
```{r Loan Data Summary, echo=FALSE, message=FALSE, warning=FALSE}
#str(ld)
#summary(ld)
na.omit(ld$CreditRating)
summary((ld$CreditRating))
```
...
ggplot(aes(x=CreditScore, y=BorrowerAPR, color=I('#099DD9')), data=ld) + geom_jitter(alpha=1/30) + 
...
ggplot(aes(y=LoanOriginalAmount, x=CreditScore, color=I('#099DD9')), data=ld) + geom_jitter(alpha=1/30) +
  scale_x_continuous() + scale_y_continuous(breaks = seq(0,40000,5000)) + coord_trans(y="log10",x="log10")
```
...
ggplot(aes(y=LoanOriginalAmount, x=CreditScore, color=I('#099DD9')), data=ld) + geom_jitter(alpha=1/30) +
  scale_x_continuous() + scale_y_log10(breaks = seq(0,40000,5000))
```
...
```{r Principa vs APR vs Color, echo=FALSE, message=FALSE, warning=FALSE}
...
### Description One
 ...
 The above graph is a simple histogram, but adding the division by Credit Rating
